...
class ConstructorResolver {
	 ...
	 * 函数中某个值为原始类型String 类型，或者是-一个对其他bean 的引用，而这一-处理委托给
	 ...
	 * 先对构造函数按照public构造函数优先参数数量降序、非pubic构造函数参数数量降序。这样
	 * 可以在遍历的情况下迅速判断排在后面的构造函数参数个数是否符合条件。
	 * 由于在配置文件中并不是唯一限制使用参数位置索引的方式去创建，同样还支持指定参数
	 ...
	 * 构造函数，不同构造函数的参数为父子关系，所以Spring在最后又做了一次验证。
	 * 5.根据实例化策略以及得到的构造函数及构造函数参数实例化Bean。后面章节中将进
	 * 行讲解。
	 */
	public BeanWrapper autowireConstructor(String beanName, RootBeanDefinition mbd,
		...
		try {
			...
			if (System.getSecurityManager() != null) {
				...
				/**
				 * @see SimpleInstantiationStrategy#instantiate(RootBeanDefinition, String, BeanFactory, Constructor, Object...)
				 */
