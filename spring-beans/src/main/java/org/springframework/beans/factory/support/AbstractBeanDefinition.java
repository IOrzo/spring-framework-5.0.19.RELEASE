 ...
 * 以及 GenericBeanDefinition。 三种实现均继承了 AbstractBeanDefinition， 其中 BeanDefinition 是配置
 * 文件 <bean> 元素标签在容器中的内部表示形式。<bean> 元素标签拥有 class、scope、lazy-init 等配置属性，
 * BeanDefinition 则提供了相应的 beanClass、scope 、lazyInit 属性，BeanDefinition 和 <bean> 中的属性
 * 是一一对应的。 其中 RootBeanDefinition 是最常用的实现类，它对应一般性的 <bean> 元素标签，
 ...
 * 而没有父 <bean> 的 <bean> 就使用 RootBeanDefinition 表示。AbstractBeanDefinition 对两者共同的类信息进行抽象。
 * Spring 通过 BeanDefinition 将配置文件中的 <bean> 配置信息转换为容器的内部表示，并将这些 BeanDefiniton 注册到
 * BeanDefinitonRegistry 中。Spring 容器的 BeanDefinitionRegstry 就像是 Spring 配置信息的内存数据库，主要是以
 * map 的形式保存，后续操作直接从 BeanDefinitionRegistry 中读取配置信息。
 */
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 用来表示一个 bean 的实例化依靠另一个 bean 先实例化, 对应bean的属性 depends-on
	 */
	@Nullable
